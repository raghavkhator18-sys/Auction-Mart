    ...
    return (
      <form onSubmit={handlePasswordSubmit} className="space-y-4">
        ...
        <div className="space-y-1">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full text-xs pl-8 pr-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-hidden focus:border-blue-600 focus:ring-1 focus:ring-blue-600 text-slate-900 dark:text-white bg-slate-50 dark:bg-slate-800/50"
            />
          </div>
        </div>
        ...
        <button
          ...
          className="w-full py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs uppercase tracking-wider rounded-xl transition-all flex items-center justify-center gap-1.5 cursor-pointer shadow-xs disabled:opacity-50"
        >
          {loading ? 'Updating Passkey...' : 'Set New Passkey'} <ArrowRight size={13} />
        </button>
      </form>
    );
  }
